...
export default defineEventHandler(async (event) => {
	...
	const arrays = generateArrays(1000, config.random as RandomConfig);
	...
	try {
		...
		return {
			...
			body: {
				...
				notification: getToast(Toasts.SUCCESS, "Тест на добавление 1000 массивов выполнен успешно за " + time),
			},
		};
	} catch (error: any) {
		return {
