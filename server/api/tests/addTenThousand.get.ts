...
export default defineEventHandler(async (event) => {
	...
	const arrays = generateArrays(10000, config.random as RandomConfig);
	...
	try {
		...
		return {
			...
			body: {
				...
				notification: getToast(Toasts.SUCCESS, "Тест на добавление 10000 массивов выполнен успешно за " + time),
			},
		};
	} catch (error: any) {
		return {
