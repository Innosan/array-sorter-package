...
export default defineEventHandler(async (event) => {
	...
	const arraysCount = getRouterParam(event, "arraysCount") as number;

	const startTime = process.hrtime();

	const arrays = generateArrays(arraysCount, config.random as RandomConfig);
	...
	try {
		...
		try {
			const res = await client.query(`SELECT * FROM public.array ORDER BY RANDOM() LIMIT $1`, [100]);
			const arrays = res.rows.map((row: { unsorted: number[] }) => row.unsorted);

			for (const array of arrays) {
				const medianSortTime = mergeSort(array).time;
			}
		} finally {
			clientSort.release();
		}
		...
		return {
			...
			body: {
				...
				notification: getToast(
					Toasts.SUCCESS,
					`Комплексный тест на добавление ${arraysCount} массивов, сортировку 100 массивов и очистку базы данных выполнен успешно за ` +
						time,
				),
			},
		};
